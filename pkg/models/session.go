...
import (
    ...
    "github.com/chrisdamba/simstreamdata/config"
)
...
func (s *Session) IncrementEvent(rng *rand.Rand, config *Config) {
    now := time.Now()
...
func (s *Session) handleNextAudioEvent(rng *rand.Rand, config *Config) {
    // Audio event logic
    if s.SubscriptionTier == Free {
        // Check probability for an ad after a song
        if rng.Float64() < config.AdFrequency.Audio {
...
func (s *Session) handleNextVideoEvent(rng *rand.Rand, config *Config) {
...
func (s *Session) handleAdEvent(rng *rand.Rand, config *Config) {
...
func (s *Session) startAdSequence(contentType string, rng *rand.Rand, config *Config) {
...
func (s *Session) scheduleNextEvent(eventType string, rng *rand.Rand, config *Config) {
...
func shouldInsertAd(s *Session, config *Config) bool {
